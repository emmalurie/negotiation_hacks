...
class TestDefStorageClass(testcase.GsUtilIntegrationTestCase):
  ...
  def test_set_and_get_for_one_bucket(self):
    ...
    stderr = self.RunGsUtil(
        self._set_dsc_cmd + [new_storage_class, suri(bucket_uri)],
        return_stderr=True)
    self.assertRegexpMatchesWithFlags(
        stderr, r'Setting default storage class to "%s" for bucket %s' % (
  ...
  def test_set_and_get_for_multiple_buckets(self):
    ...
    stderr = self.RunGsUtil(
        self._set_dsc_cmd + [
            new_storage_class, suri(bucket1_uri), suri(bucket2_uri)],
        return_stderr=True)
    ...
    stdout = self.RunGsUtil(
        self._get_dsc_cmd + [suri(bucket1_uri), suri(bucket2_uri)],
        return_stdout=True)
    for bucket_uri in (suri(bucket1_uri), suri(bucket2_uri)):
      self.assertRegexpMatchesWithFlags(
